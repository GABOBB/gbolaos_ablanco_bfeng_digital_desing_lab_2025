module videoGen (
    ...
    input logic rst_n,            // Reset activo en nivel bajo
    input logic [9:0] x_pos, y_pos, // Posición actual del píxel
    input logic [83:0] grid_state, // Estado actual del tablero (6x7 celdas)
    input logic [3:0] current_state, // Estado de la máquina de estados del juego
    input logic [2:0] selected_col, // Columna seleccionada por el jugador actual
    input logic [23:0] winning_line, // Coordenadas de la línea ganadora

    output logic [7:0] red, green, blue // Señales de color RGB
);
    ...
    localparam logic [10:0] board_width = 7 * cell_size + 6 * gap; // Ancho total
    localparam logic [10:0] board_height = 6 * cell_size + 5 * gap; // Alto total
    localparam logic [10:0] start_x = 11'd100;   // Posición X inicial
    localparam logic [10:0] start_y = 11'd50;    // Posición Y inicial
    // Características del texto de victoria
    localparam TEXT_LETTER_WIDTH = 10'd20;      // Ancho de cada letra
    localparam TEXT_LETTER_HEIGHT = 10'd30;     // Alto de cada letra
    localparam TEXT_LETTER_SPACING = 10'd5;     // Espacio entre letras
    localparam STROKE_THICKNESS = 3;            // Grosor del trazo
    
    // Posicionamiento vertical del texto
    localparam TEXT_Y_TOP = start_y - TEXT_LETTER_HEIGHT - 15; 
    localparam TEXT_Y_BOTTOM = TEXT_Y_TOP + TEXT_LETTER_HEIGHT;
    
    // Cálculo de posición horizontal centrada
    ...
    // Señales para identificar elementos en pantalla
    logic board_area;          // Área del tablero completo
    logic [2:0] current_col, visual_row, data_row; // Posiciones convertidas
    logic cell_area;           // Dentro de una celda específica
    logic player1_chip, player2_chip; // Fichas de jugadores
    logic winning_chip;        // Ficha parte de línea ganadora
    logic highlight_col;       // Columna actualmente seleccionada
    logic preview_chip;        // Previsualización de ficha
    logic board_frame;         // Marco alrededor del tablero
    
    // Control de pantallas especiales
    logic is_initial_screen;   // Pantalla de inicio activa
    logic in_title_area_start_screen; // Área de título en inicio
    logic show_win_text;       // Mostrar texto de victoria
    ...
    localparam TITLE_RECT_Y_START = start_y + board_height/3; 
    ...
    // Mapeo de coordenadas visuales a posiciones en la matriz del juego
    assign current_col = (x_pos >= start_x && x_pos < start_x + board_width) ? 
                        (x_pos - start_x) / (cell_size + gap) : 3'bxxx;
    assign visual_row = (y_pos >= start_y && y_pos < start_y + board_height) ? 
                       (y_pos - start_y) / (cell_size + gap) : 3'bxxx;
    assign data_row = 5 - visual_row; // Inversión para matriz interna
    ...
    always_comb begin
        ...
        // Paso 3: Si NO es pantalla de inicio, dibujar elementos del juego y/o pantalla de Game Over
            ...
            // B. Superponer highlight y preview 
